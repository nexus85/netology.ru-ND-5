...
angular.module('myApp.view1', ['ngRoute'])
    ...
    .controller('View1Ctrl', ['filterFilter', 'orderByFilter', '$scope', function (filterFilter, orderByFilter, $scope) {
        ...
        $scope.nameAlert = name => window.alert(name);
        ...
        $scope.result = $scope.pokemons;

        $scope.myFilter = () => {
            let queryFilter = filterFilter($scope.pokemons, $scope.myQuery);
            let typeFilter = filterFilter(queryFilter, {
                type: $scope.myTypeFilter
            });

            $scope.result = orderByFilter(typeFilter, $scope.myOrderProperty);
        };
